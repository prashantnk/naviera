...
export function AddressFieldset({ control, type, title, savedAddresses = [] }: AddressFieldsetProps) {
    const [mode, setMode] = useState<"saved" | "manual">("saved");

    const idField = type === "pickup" ? "pickup_address_id" : "delivery_address_id";
    const objectField = type === "pickup" ? "new_pickup_address" : "new_delivery_address";
        ...
        <div className="space-y-6 bg-slate-50/50 p-6 rounded-xl border border-slate-100">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                ...
                <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200 w-fit">
                    <Button
                        type="button" variant={mode === "saved" ? "default" : "ghost"} size="sm"
                        onClick={() => setMode("saved")} className="h-8"
                    >
                        <BookUser className="mr-2 h-4 w-4" /> Address Book
                    </Button>
                    <Button
                        type="button" variant={mode === "manual" ? "default" : "ghost"} size="sm"
                        onClick={() => setMode("manual")} className="h-8"
                    >
            ...
            {mode === "saved" && (
                <div className="animate-in fade-in slide-in-from-left-4">
                    <FormField control={control} name={idField} render={({ field }) => (
                        <FormItem>
                            <FormLabel>Select a Saved Address</FormLabel>
                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                <FormControl>
                                    <SelectTrigger className="bg-white"><SelectValue placeholder="Choose from address book..." /></SelectTrigger>
                                </FormControl>
                                <SelectContent>
                                    {savedAddresses.length === 0 && (
                                        <div className="p-2 text-sm text-slate-500 italic">No addresses saved yet.</div>
                                    )}
                                    {savedAddresses.map((addr) => (
                                        <SelectItem key={addr.id} value={addr.id}>
                                            <span className="font-medium text-slate-900">{addr.name}</span>
                                            <span className="text-slate-400 ml-2">({addr.city}, {addr.state})</span>
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
            ...
            {mode === "manual" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-right-4">
                    <FormField control={control} name={`${objectField}.name`} render={({ field }) => (
                    ...
                    <FormField control={control} name={`${objectField}.phone`} render={({ field }) => (
                    ...
                    <FormField control={control} name={`${objectField}.address_line1`} render={({ field }) => (
                    ...
                    <FormField control={control} name={`${objectField}.city`} render={({ field }) => (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <FormField control={control} name={`${objectField}.state`} render={({ field }) => (
                        ...
                        <FormField control={control} name={`${objectField}.pincode`} render={({ field }) => (
