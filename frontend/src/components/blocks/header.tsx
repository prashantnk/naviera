...
import { LayoutDashboard, LogOut, Menu, Package2, PhoneCall, UserCircle2, X } from "lucide-react";
      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white shadow-sm relative">
        <div className="container mx-auto px-4 md:px-6 flex h-[72px] items-center justify-between">
          ...
          <Link href={routeTo("/")} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            {brand?.logo_url ? (
              <img src={brand.logo_url} alt={tenant?.name || "Naviera"} className="h-10 md:h-12 object-contain" />
            ) : (
              <>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-white border-b border-slate-200 shadow-xl py-6 px-6 flex flex-col gap-6 animate-in slide-in-from-top-2">
            <nav className="flex flex-col gap-4 text-base font-bold text-slate-700">
              <Link href={routeTo("/")} className="hover:text-primary transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Home</Link>
              <Link href={routeTo("/about")} className="hover:text-primary transition-colors" onClick={() => setIsMobileMenuOpen(false)}>About Us</Link>
              <Link href={routeTo("/services")} className="hover:text-primary transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Services</Link>
              <Link href={routeTo("/track")} className="text-primary hover:text-primary/80 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Track Shipment</Link>
              <Link href={routeTo("/contact")} className="hover:text-primary transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Contact Us</Link>
            </nav>

            <div className="flex flex-col gap-3 pt-6 border-t border-slate-100">
              {isLoggedIn ? (
                <>
                  <Button variant="outline" size="lg" asChild className="w-full justify-start border-slate-300">
                    <Link href={routeTo("/dashboard")}><LayoutDashboard className="h-5 w-5 mr-2" /> Dashboard</Link>
                  </Button>
                  <Button variant="ghost" size="lg" onClick={handleLogout} className="w-full justify-start text-red-600 hover:text-red-700 hover:bg-red-50">
